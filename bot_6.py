...
def db():
    ...
    con.execute("CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY, balance REAL DEFAULT 0)")
    con.execute("CREATE TABLE IF NOT EXISTS transactions (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, invoice_id TEXT UNIQUE, amount REAL, status TEXT DEFAULT 'active', created_at TEXT DEFAULT (datetime('now')))")
    con.commit()
    return con

async def api(method, **kw):
    async with aiohttp.ClientSession() as s:
        async with s.get(f"{CRYPTO_API}/{method}", params=kw, headers={"Crypto-Pay-API-Token": CRYPTO_TOKEN}) as r:
            return await r.json()
...
async def make_inv(cb: CallbackQuery, bot: Bot):
    ...
    inv = await api("createInvoice", asset=ASSET, amount=str(amount), custom=str(cb.from_user.id), expires_in=3600)
    if not inv.get("ok"):
